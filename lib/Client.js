...
Client.prototype.nodeEscape = function(val, stringifyObjects) {
  ...
  if (typeof val === 'object') {
    if (stringifyObjects) {
      ...
      return this.objectToValues(val);
    }
  }
...
Client.prototype.arrayToList = function(array) {
  return array.map(function(v) {
    if (Array.isArray(v)) return '(' + Client.prototype.arrayToList(v) + ')';
    return Client.prototype.escape(v, true);
  }).join(', ');
};
...
Client.prototype.format = function(sql, values) {
  ...
  return sql.replace(/\?/g, function(match) {
    ...
    return Client.prototype.nodeEscape(values.shift());
  });
};

Client.prototype.dateToString = function(date) {
...
Client.prototype.bufferToString = function(buffer) {
  var hex = '';
  try {
...
Client.prototype.objectToValues = function(object) {
  var values = [];
  for (var key in object) {
    ...
    values.push('`' + key + '` = ' + Client.prototype.nodeEscape(value, true));
  }
      ...
      fn = function(values) {
        ...
        for (j=0,len=tokens.length; j<len; ++j) {
          if (Buffer.isBuffer(values[tokens[j]]))
            ...
            str = values[tokens[j]] + '';
          ret += parts[j];
          ret += Client.prototype.nodeEscape(str);
        }
        if (j < parts.length)
          ret += parts[j];
        return ret;
